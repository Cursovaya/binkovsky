...
def create_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS hotel_images
                 (id INTEGER PRIMARY KEY, hotel_id INTEGER, image_path TEXT,
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS room_images
                 (id INTEGER PRIMARY KEY, room_id INTEGER, image_path TEXT,
...
def add_hotel_image(hotel_id, image_path):
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("INSERT INTO hotel_images (hotel_id, image_path) VALUES (?, ?)",
              (hotel_id, image_path))
    conn.commit()
    conn.close()
...
def get_hotel_images(hotel_id):
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT image_path FROM hotel_images WHERE hotel_id = ?", (hotel_id,))
    images = c.fetchall()
    conn.close()
    return images
...
def add_room_image(room_id, image_path):
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("INSERT INTO room_images (room_id, image_path) VALUES (?, ?)",
              (room_id, image_path))
    conn.commit()
    conn.close()
...
def get_room_images(room_id):
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT image_path FROM room_images WHERE room_id = ?", (room_id,))
    images = c.fetchall()
    conn.close()
    return images
...
class LoginWindow:
    def __init__(self, root):
        ...
        self.canvas.create_rectangle(497.0, 170.0, 1389.0, 911.0, fill="#FFFFFF", outline="")
        ...
        self.entry_bg_1 = self.canvas.create_image(1082.0, 418.5, image=self.entry_image_1)
        ...
        self.login_entry.place(x=862.0, y=393.0, width=440.0, height=49.0)

        self.entry_image_2 = tk.PhotoImage(file=relative_to_assets("entry_2.png"))
        self.entry_bg_2 = self.canvas.create_image(1087.5, 554.0, image=self.entry_image_2)
        ...
        self.password_entry.place(x=862.0, y=530.0, width=451.0, height=46.0)
        ...
        self.button_1.place(x=568.0, y=655.0, width=303.0, height=72.0)
        ...
        self.button_2.place(x=1064.0, y=655.0, width=259.0, height=72.0)
        ...
        self.button_3.place(x=753.0, y=768.0, width=414.0, height=59.0)
    ...
    def user_dashboard(self, first_name, last_name):
        ...
        tk.Label(self.user_frame, text=f"Добро пожаловать, {first_name} {last_name}").grid(row=0, column=0,
                                                                                           columnspan=2)

        tk.Button(self.user_frame, text="Забронировать номер", command=self.book_room).grid(row=1, column=0)
        tk.Button(self.user_frame, text="Проверить бронь", command=self.check_booking).grid(row=1, column=1)

    def admin_dashboard(self):
        self.admin_frame = tk.Frame(self.root)
        self.admin_frame.pack()

        tk.Label(self.admin_frame, text="Добро пожаловать, администратор").grid(row=0, column=0, columnspan=2)

        tk.Button(self.admin_frame, text="Список отелей", command=self.manage_hotels).grid(row=1, column=0)
        tk.Button(self.admin_frame, text="Список пользователей", command=self.manage_users).grid(row=1, column=1)
        tk.Button(self.admin_frame, text="Проверить бронь", command=self.check_all_bookings).grid(row=2, column=0,
                                                                                                  columnspan=2)
    ...
    def show_hotel_details(self, hotel):
        ...
        details_frame.geometry("600x500")
        ...
        for i, image_path in enumerate(images):
            try:
                ...
                image_label.grid(row=i, column=0, columnspan=2, pady=10)
            except Exception as e:
                messagebox.showerror("Ошибка", f"Не удалось загрузить изображение: {e}")

        # Отображение описания отеля
        tk.Label(details_frame, text=f"Адрес: {hotel[2]}").grid(row=len(images), column=0, columnspan=2, pady=10)
        tk.Label(details_frame, text=f"Описание: {hotel[3]}").grid(row=len(images) + 1, column=0, columnspan=2, pady=10)

        # Отображение списка номеров
        rooms = get_rooms(hotel[0])
        for i, room in enumerate(rooms):
            tk.Button(details_frame, text=f"Номер: {room[2]}", command=lambda r=room: self.room_details(r)).grid(
                row=len(images) + 2 + i, column=0, pady=10)
            tk.Label(details_frame, text=f"Цена: {room[4]} руб.").grid(row=len(images) + 2 + i, column=1, pady=10)

        tk.Button(details_frame, text="Назад", command=details_frame.destroy).grid(row=len(images) + len(rooms) + 3,
    ...
    def save_hotel(self, name, address, description, image_path, frame):
        ...
        if image_path:
            ...
            new_image_path = f"hotel_images/{uuid4()}.jpg"
            os.makedirs(os.path.dirname(new_image_path), exist_ok=True)
            copyfile(image_path, new_image_path)
            add_hotel_image(hotel_id, new_image_path)
    ...
    def edit_hotel(self, hotel_id):
        ...
        def save_changes():
            ...
            if image_path_var.get():
                ...
                # Сохраняем новую фотографию в папку
                new_image_path = f"hotel_images/{uuid4()}.jpg"
                os.makedirs(os.path.dirname(new_image_path), exist_ok=True)
                copyfile(image_path_var.get(), new_image_path)

                # Добавляем путь к новой фотографии в базу данных
                c.execute("INSERT INTO hotel_images (hotel_id, image_path) VALUES (?, ?)",
                          (hotel_id, new_image_path))
    ...
    def save_room(self, hotel_id, number, description, price, image_path, frame):
        ...
        if image_path:
            # Сохраняем фотографию в папку
            new_image_path = f"room_images/{uuid4()}.jpg"
            os.makedirs(os.path.dirname(new_image_path), exist_ok=True)
            copyfile(image_path, new_image_path)
            add_room_image(room_id, new_image_path)
    ...
    def add_user(self):
        ...
        tk.Label(add_user_frame, text="Фамилия:").grid(row=1, column=0, pady=10)
        last_name_entry = tk.Entry(add_user_frame)
        last_name_entry.grid(row=1, column=1, pady=10)

        tk.Label(add_user_frame, text="Отчество:").grid(row=2, column=0, pady=10)
        middle_name_entry = tk.Entry(add_user_frame)
        middle_name_entry.grid(row=2, column=1, pady=10)

        tk.Label(add_user_frame, text="Телефон:").grid(row=3, column=0, pady=10)
        phone_entry = tk.Entry(add_user_frame)
        phone_entry.grid(row=3, column=1, pady=10)

        tk.Label(add_user_frame, text="Email:").grid(row=4, column=0, pady=10)
        email_entry = tk.Entry(add_user_frame)
        email_entry.grid(row=4, column=1, pady=10)

        tk.Label(add_user_frame, text="Логин:").grid(row=5, column=0, pady=10)
        login_entry = tk.Entry(add_user_frame)
        login_entry.grid(row=5, column=1, pady=10)

        tk.Label(add_user_frame, text="Пароль:").grid(row=6, column=0, pady=10)
